...
- 상황추천 상세 해설 (자세히 보기: 조건 정합성/실행 체크리스트/적용 규칙)
- 상황추천 상세 UI는 정보 과밀을 피하고, 필요한 내용만 단계적으로 펼쳐 보이도록 구성
- 기술 비교 페이지 (직무 전환 + 다중 선택 비교)
- 트렌드 브리핑 페이지 (직무/기간/영향도 필터)
- 관심리스트 페이지 (로컬 저장 기반)
- 월간 인사이트 페이지
- UI/UX 고도화 1차: 타이포 스케일/의미 기반 텍스트 토큰/포커스 가시성/터치 타깃/밀집 테이블 가독성 개선
- UI/UX 고도화 2차: 명확한 한 줄 결론, 요약 우선 구조, 인지부하 감소(토스식 clarity-first 원칙 참고)
